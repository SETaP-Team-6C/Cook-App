...
def test_complete_step_missing_user_id(client):
    # Authenticate first (reuse test user)
    body = {
        "user_fname": "test",
        "user_lname": "user",
        "user_password": "123456"
    ...
    # Add three recipes: Test Recipe, Test Recipe 2, Test Recipe 3
    for i, title in enumerate(["Test Recipe", "Test Recipe 2", "Test Recipe 3"], start=1):
        data = {
            "recipe-ingredients": json.dumps([
                {
                    "ingredient-name": "Ingredient 1",
        ...
        assert response.status_code == 201
        assert response.mimetype == "application/json"
        assert response.get_json()["recipe-id"] == i

    # Search using the encoded uppercase query as requested
    response = client.get("/search-recipe?q=TEST%20RECIPE")
    assert response.status_code == 200
    assert response.mimetype == "application/json"
    assert response.get_json() == ({
